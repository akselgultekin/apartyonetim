<x-layouts.app heading="Gelirler" subheading="Kira, depozito, hizmet ve diger gelir kayitlari">
    ...
    <div class="grid gap-5 xl:grid-cols-[.8fr_1.2fr]">
        <form method="post" action="{{ route('incomes.store') }}" class="rounded-md border border-slate-200 bg-white p-5">
            @csrf
            <h2 class="mb-4 font-semibold">Yeni gelir</h2>
            <div class="grid gap-4 md:grid-cols-2">
                <x-input name="title" label="Baslik" />
                <x-select name="type" label="Tur">@foreach(['daily_rent'=>'Gunluk kira','weekly_rent'=>'Haftalik kira','monthly_rent'=>'Aylik kira','yearly_rent'=>'Yillik kira','deposit'=>'Depozito','service'=>'Ek hizmet','other'=>'Diger'] as $k=>$v)<option value="{{ $k }}">{{ $v }}</option>@endforeach</x-select>
                <x-input name="amount" label="Tutar" type="number" step="0.01" value="0" />
                <x-input name="paid_amount" label="Odenen" type="number" step="0.01" value="0" />
                <x-input name="date" label="Tarih" type="date" :value="now()->toDateString()" />
                <x-select name="payment_method" label="Yontem">@foreach(['cash'=>'Nakit','bank_transfer'=>'Havale/EFT','credit_card'=>'Kredi karti','other'=>'Diger'] as $k=>$v)<option value="{{ $k }}">{{ $v }}</option>@endforeach</x-select>
                ...
                <x-select name="customer_id" label="Musteri"><option value="">Secilmedi</option>@foreach($customers as $customer)<option value="{{ $customer->id }}">{{ $customer->full_name }}</option>@endforeach</x-select>
        ...
        <section class="overflow-hidden rounded-md border border-slate-200 bg-white">
            <table class="w-full text-left text-sm"><thead class="bg-slate-100 text-slate-600"><tr><th class="p-3">Gelir</th><th class="p-3">Iliski</th><th class="p-3">Tarih</th><th class="p-3">Odeme</th></tr></thead><tbody>@foreach($incomes as $income)<tr class="border-t border-slate-100"><td class="p-3"><strong>{{ $income->title }}</strong><div class="text-slate-500">{{ $income->type }}</div></td><td class="p-3">{{ $income->location?->name }}<div class="text-slate-500">{{ $income->room?->name }} {{ $income->customer?->full_name }}</div></td><td class="p-3">{{ $income->date->format('d.m.Y') }}</td><td class="p-3">{{ $money($income->paid_amount) }}<div class="text-slate-500">{{ $income->payment_status }}</div></td></tr>@endforeach</tbody></table>
