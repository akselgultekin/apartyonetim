<x-layouts.app heading="Giriş / Çıkış" subheading="Tarih aralığı çakışmasını engelleyen konaklama kayıtları">
    <x-filter-bar>
        ...
        <select name="customer_id" class="h-10 rounded-md border border-slate-300 px-3 text-sm"><option value="">Tüm müşteriler</option>@foreach($customers as $customer)<option value="{{ $customer->id }}" @selected(request('customer_id')==$customer->id)>{{ $customer->full_name }}</option>@endforeach</select>
    ...
    <div class="grid gap-5 xl:grid-cols-[.85fr_1.15fr]">
        <form method="post" action="{{ route('stays.store') }}" class="rounded-md border border-slate-200 bg-white p-5">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <x-select name="customer_id" label="Müşteri">@foreach($customers as $customer)<option value="{{ $customer->id }}">{{ $customer->full_name }}</option>@endforeach</x-select>
                ...
                <x-input name="check_in" label="Giriş tarihi" type="date" :value="now()->toDateString()" />
                <x-input name="check_out" label="Çıkış tarihi" type="date" :value="now()->addDay()->toDateString()" />
                <x-select name="rental_type" label="Kiralama türü">@foreach(['daily'=>'Günlük','weekly'=>'Haftalık','monthly'=>'Aylık','yearly'=>'Yıllık'] as $k=>$v)<option value="{{ $k }}">{{ $v }}</option>@endforeach</x-select>
                <x-input name="total_rent" label="Toplam kira" type="number" step="0.01" value="0" />
                <x-input name="paid_amount" label="Ödenen tutar" type="number" step="0.01" value="0" />
