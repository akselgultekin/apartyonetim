<x-layouts.app heading="Doluluk Takvimi" subheading="Oda ve lokasyon bazlı tarih aralığı görünümü">
    @php
        $days = collect();
        for ($date = $start->copy(); $date->lte($end); $date->addDay()) { $days->push($date->copy()); }
    @endphp
    <form method="get" class="shell-card mb-5 flex flex-wrap gap-3 rounded-md border border-slate-200 bg-white p-4">
        ...
        <button class="h-10 rounded-md bg-slate-900 px-4 text-sm font-semibold text-white">Göster</button>
    </form>
    <section class="shell-card overflow-auto rounded-md border border-slate-200 bg-white">
