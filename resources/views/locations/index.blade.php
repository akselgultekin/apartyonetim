<x-layouts.app heading="Lokasyonlar" subheading="Apart/adres bazli operasyon ve finans ayrimi">
    <x-filter-bar>
    ...
    <div class="grid gap-5 xl:grid-cols-[.8fr_1.2fr]">
        <form method="post" action="{{ $editing ? route('locations.update', $editing) : route('locations.store') }}" class="rounded-md border border-slate-200 bg-white p-5">
            @csrf
            <h2 class="mb-4 font-semibold">{{ $editing ? 'Lokasyon duzenle' : 'Yeni lokasyon' }}</h2>
            <div class="grid gap-4">
                ...
                <x-textarea name="description" label="Aciklama" :value="$editing?->description" />
            ...
            <table class="w-full text-left text-sm">
                ...
                @foreach($locations as $location)
                    <tr class="border-t border-slate-100">
                        ...
                        <td class="p-3 text-right"><a class="font-medium text-slate-700" href="{{ route('locations.index', ['edit' => $location->id]) }}">Duzenle</a></td>
                    </tr>
                @endforeach
                </tbody>
            </table>
